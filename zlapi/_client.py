...
class ZaloAPI(object):
	...
	def sendRemoteFile(self, fileUrl, thread_id, thread_type, fileName="default", fileSize=None, extension="vrxx"):
		...
		Raises:
			ZaloAPIException: If request failed
		"""
		import hashlib  # Đảm bảo hashlib được import

		# Luôn lấy nội dung file để tính checksum và kích thước (nếu fileSize không được cung cấp)
		try:
			with self._state._session.get(fileUrl) as response:
				if response.status_code == 200:
					if fileSize is None:
						fileSize = int(response.headers.get("Content-Length", 0)) or len(response.content)
					fileChecksum = hashlib.md5(response.content).hexdigest()
				else:
					raise ZaloAPIException(f"Unable to get url content, status code: {response.status_code}")
		except Exception as e:
			raise ZaloAPIException(f"Unable to get url content: {str(e)}")
	...
	def sendLocalImage(self, imagePath, thread_id, thread_type, width=2560, height=2560, message=None):
		...
		error_message = data.get("error_message") or data.get("data")
		raise ZaloAPIException(f"Error #{error_code} when sending requests: {error_message}")
	def sendRemoteVoice(self, fileUrl, thread_id, thread_type, fileName="default", fileSize=None, duration=None, extension="mp3"):
		"""Send a Voice Message to a User/Group with url.
			
		Args:
			fileUrl (str): URL of the voice file to send
			thread_id (int | str): User/Group ID to send to
			thread_type (ThreadType): ThreadType.USER, ThreadType.GROUP
			fileName (str): File name to send (default: "default")
			fileSize (int): File size to send (default: None, will be fetched from URL)
			duration (int): Duration of the voice file in seconds (default: None)
			extension (str): File extension (default: "mp3")
		...
		# Lấy nội dung file để tính fileSize và fileChecksum
		try:
			with self._state._session.get(fileUrl) as response:
				if response.status_code == 200:
					if fileSize is None:
		...
		payload = {
			"params": {
				...
				"fileName": fileName,
				"clientId": _util.now(),
				"fType": 2,  # Giả định fType=2 cho voice message
				"fileCount": 0,
				"fdata": "{}",  # Có thể cần điều chỉnh nếu Zalo yêu cầu thêm dữ liệu
				"fileUrl": fileUrl,
				"zsource": 402,  # Giả định zsource=402 cho voice message
				"ttl": 0,
				"duration": duration if duration is not None else 0  # Thêm duration cho voice message
			}
		}

		if thread_type == ThreadType.USER:
			url = "https://tt-files-wpa.chat.zalo.me/api/message/voice"
			payload["params"]["toid"] = str(thread_id)
			payload["params"]["imei"] = self._imei
		elif thread_type == ThreadType.GROUP:
			url = "https://tt-files-wpa.chat.zalo.me/api/group/voice"
			payload["params"]["grid"] = str(thread_id)
		else:
			raise ZaloUserError("Thread type is invalid")
